...
function FormulaEditAreaLoaded(sTextAreaId)
{
  ...
  $changesButton.click(function()
  {
	...
	hiddenBaseURL.value = document.getElementById("ForceFormulaEditorBaseURL").value; //get the value from the existing hidden that the contentscript added to the current page, we will create a duplicate in the popup
	...
	var sLoaderPath = document.getElementById("editorLoaderScript").getAttribute("src"); //get the value from the existing script that the contentscript added to the current page, we will create a duplicate in the popup
	var sJS = `
	...
	{	
		...
		editorJQuery("#old", newWin.document).next("iframe").contents().find("#content_changes").html(sOldCodeDiffHTML);

		var sNewCodeDiffHTML = GetHTML(diff, [DIFF_INSERT, DIFF_EQUAL]);
		editorJQuery("#new", newWin.document).next("iframe").contents().find("#content_changes").html(sNewCodeDiffHTML);
		
		editorJQuery("#old", newWin.document).next("iframe").contents().find("#content_changes").find("del").each(function()
		{
			editorJQuery(this).parent().addClass("hasDel");
		});

		editorJQuery("#new", newWin.document).next("iframe").contents().find("#content_changes").find("ins").each(function()
		{
			editorJQuery(this).parent().addClass("hasIns");
		});
	}//end LoadDiffs
  });
