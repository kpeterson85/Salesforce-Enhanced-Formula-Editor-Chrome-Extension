...
function init() {
	...
	function LoadEditorFiles(elWindow, elDocument, elTextField)
	{
		...
		chrome.storage.sync.get(['FormulaEditorLicense'], function(result)
		{
			...
			var sOptionsUrl = chrome.runtime.getURL("options.html");
			...
			if (sFormulaEditorLicense != null && sFormulaEditorLicense.trim() != "")
			{
				...
				.then(function (data)
				{
					...
					{
						//THIS IS CALLED HERE AND IN THE CATCH FUTHER DOWN
						LicenseKeyCheckSuccess(elWindow, elDocument);						
					}
					else
					{
				...
				.catch(function (err)
				{
					//COMMENTED OUT 7/31/2022 SO THAT IF THE LICENSE KEY CHECK URL IS NOT WORKING PEOPLE CAN CONTINUE TO USE THE EXTENSION
			...
			{
				eStatusMessage.innerHTML = "<b>Enhanced Formula Editor Extension - Action Required</b><br>This extension requires a subscription license key. Click the 'Get a License Key' button below and follow the 3-step process to get started (includes a 14-day free trial).<br>" + sGetLicenseKeyButton + sOptionsButton;
				eStatusMessage.style.display = "block";
			}
		});			
...
function LicenseKeyCheckSuccess(elWindow, elDocument)
{
	...
	{							
		...
		function FinishLoadingFiles()
		{
			...
			hiddenBaseURL.value = chrome.runtime.getURL("");
			...
			{								
				...
				jqueryScript.src = chrome.runtime.getURL("jquery.min.js");
				...
				jsforceScript.src = chrome.runtime.getURL("jsforce-core.min.js");
				...
				{								
					...
					activate.src = chrome.runtime.getURL("activate_editor.js");
					...
					elWindow.addEventListener("message", function(event)
					{
						if (event.data.hasOwnProperty("type") && event.data.type == "FormulaEditorReviewChanges")
						{
							...
							var script = elDocument.createElement("script");
							script.type = "text/javascript";
							script.src = chrome.runtime.getURL("review.js");
